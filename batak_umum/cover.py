...
def extract_cover(doc):
    ...
    tata_ibadah = "PARTURENA PARMINGGUON"
    nama_minggu = ""
    topik = ""
    tanggal = ""
    ...
    for i, text in enumerate(paragraphs[:25]):
        ...
        if "TOPIK" in text_upper:
            # Cek apakah topik ada di baris yang sama setelah tanda titik dua
            if ":" in text and len(text.split(":", 1)[1].strip()) > 1:
                topik = text.split(":", 1)[1].strip()
            # Jika baris 'TOPIK' sendirian, ambil 1-2 baris di bawahnya yang bukan gambar/kosong
            elif i + 1 < len(paragraphs):
                next_text = paragraphs[i+1]
                if "HURIA" not in next_text.upper() and len(next_text) > 3:
                    topik = next_text
