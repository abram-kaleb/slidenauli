...
if st.session_state.tata_bytes:
    ...
    with c_set2:
        # Menyamakan fungsi: Batak Umum kini memiliki pilihan Mode Tampilan seperti Indonesia Umum
        ...
        # Selectbox background dipisah agar tidak duplikat ID dan muncul untuk semua format
    ...
    if st.button("🚀 Generate Integrated PPT"):
        if selected_mode == "YouTube":
            # Memilih module stream sesuai format yang dipilih
            if selected_fmt == "Ibadah Batak Umum":
                m_ppt_module = batak_ppt_stream
            else:
                m_ppt_module = indo_ppt_stream
        else:
            m_ppt_module = m_ppt
